...
export default class Rep0st implements PoweruserModule {
  ...
  addButton(container: any) {
    ...
    if (imgElement[0] && imgElement[0].tagName !== "VIDEO") {
      const template = $(
        `<a title="Prüfe, ob es sich um einen Repost handelt" class="repost-link"><span class="fa fa-copy"></span> rep0st?</a>`
      );
  ...
  checkImage(container: any, imgElement: any) {
    ...
    let template = $(
      `<div class="sidebar-head"><span class="fa fa-copy"></span><span class="sidebar-label">Reposts</span></div>`
    );
    ...
    GM_xmlhttpRequest({
      ...
      onload: (res) => {
        ...
        for (let i = 1; i < images.length; i++) {
          let postId = parseInt(
            images[i].href.replace("pr0gramm", "").replace(/\D/g, "")
          );
  ...
  getCurrentPostId() {
    return parseInt(
      window.location.href.replace("pr0gramm", "").replace(/\D/g, "")
    );
  }
  ...
  displayImages(bar: any, urls: any) {
    for (const element of urls) {
      ...
      let container = bar.append(
        $(
          `<a href=${element.url} target="_blank"><img src=${element.img} class="rep0st-thumb" />${probabilityContainer}<span title="Als Repost markieren" class="fa fa-comment"></span></a>`
        )
      );

      let comment = container.find(
        `a[href='${element.url}'] > .fa.fa-comment`
      )[0];
