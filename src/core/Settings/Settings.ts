...
export default class Settings {
  ...
  static saveSettings(moduleList: HTMLDivElement) {
    let result = [];
    let actives: NodeListOf<HTMLElement> =
      moduleList.querySelectorAll(":checked");
  ...
  static addHint() {
    if (!document.getElementById("settings_hint")) {
      ...
      hint.innerText =
        "Bitte öffne die Einstellungen um p0weruser zu konfigurieren!";
  ...
  toggleSettingsTab(button: HTMLAnchorElement) {
    ...
    let clearButton = this.tabContent.getElementsByClassName(
    ...
    // Add save-button
    let saveButton = this.tabContent.querySelectorAll(
      "#save-addon-settings"
    )[0];
          ...
          for (const el of settings) {
            ...
            switch (el.type) {
              ...
              case "text":
                ...
                break;
              default:
                container.innerHTML = `<input id="${id}" type="${
                  el.type ? el.type : "checkbox"
                }" class="box-from-label" ${
                  currentValue ? "checked" : ""
                } /><label for="${id}">${el.title}<span>${
                  el.description
                }</span></label>`;
                break;
            }
  ...
  loadVersionInfo() {
    ...
    Settings.getVersion().then((version: string) => {
